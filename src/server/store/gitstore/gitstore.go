...
// AddVisualization creates a branch and commits the visualization summary.
func (gs *GitStore) AddVisualization(viz data.Visualization) (string, error) {
	gs.mu.Lock()
	defer gs.mu.Unlock()

	branch := fmt.Sprintf("viz/%s", viz.VisualizationID)
	dir := filepath.Join("visualizations", viz.VisualizationID)

	if err := gs.createBranch(branch); err != nil {
		return "", err
	}

	if err := gs.writeJSON(filepath.Join(dir, "summary.json"), viz); err != nil {
		return "", err
	}

	if err := gs.commitAndPush(branch, fmt.Sprintf("Add visualization %s", viz.VisualizationID)); err != nil {
		return "", err
	}
...
// SealVisualization writes nft_metadata.json, commits, pushes, and creates a PR.
func (gs *GitStore) SealVisualization(id string, nftMetadata any) (string, error) {
	prURL, err := func() (string, error) {
		gs.mu.Lock()
		defer gs.mu.Unlock()

		branch := fmt.Sprintf("viz/%s", id)

		if err := gs.checkoutBranch(branch); err != nil {
			return "", err
		}

		dir := filepath.Join("visualizations", id)
		if err := gs.writeJSON(filepath.Join(dir, "nft_metadata.json"), nftMetadata); err != nil {
			return "", err
		}

		if err := gs.commitAndPush(branch, fmt.Sprintf("Seal visualization %s with NFT metadata", id)); err != nil {
			return "", err
		}

		return gs.forge.CreatePR(
			branch, "main",
			fmt.Sprintf("Visualization: %s", id),
			fmt.Sprintf("Sealed visualization `%s` with NFT metadata.", id),
		)
	}()
	if err != nil {
		return "", fmt.Errorf("creating PR: %w", err)
