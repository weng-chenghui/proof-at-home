...
func TestVisualizationFlow(t *testing.T) {
	vizID := fmt.Sprintf("test-viz-%d", time.Now().UnixNano())

	// 1. POST /visualizations → 201 + commit_sha
	body := map[string]any{
		"visualization_id": vizID,
		"author_username":  "viz-author",
		"conjecture_id":    "prob_001",
		"domain":           "group-theory",
		"title":            "Test Visualization",
		"summary":          "A test visualization of group structure.",
		"viz_json": map[string]any{
			"summary": "Test summary",
			"visualizations": []map[string]any{
				{
					"type":        "generic_graph",
					"title":       "Morphism Graph",
					"description": "Shows group relationships",
					"data": map[string]any{
						"nodes": []map[string]any{
							{"id": "A", "label": "Group A", "group": "default"},
							{"id": "B", "label": "Group B", "group": "default"},
						},
						"edges": []map[string]any{
							{"source": "A", "target": "B", "label": "f", "directed": true},
						},
					},
				},
			},
		...
		"cost_usd":      0.02,
		"strategy_used": "visualize-group-theory",
	}
	status, resp := postJSON(t, "/visualizations", body)
	if status != http.StatusCreated {
		t.Fatalf("POST /visualizations: status %d, body %v", status, resp)
	}
	if _, ok := resp["commit_sha"]; !ok {
		t.Error("response missing commit_sha")
	}

	// 2. POST /visualizations/{id}/seal → 201 + pr_url
	nft := map[string]any{
		"name":        "Proof@Home Visualization — viz-author",
		"description": "AI-generated conjecture visualization.",
		"attributes": []map[string]any{
			{"trait_type": "Author Username", "value": "viz-author"},
			{"trait_type": "Visualization ID", "value": vizID},
			{"trait_type": "Conjecture ID", "value": "prob_001"},
			{"trait_type": "Domain", "value": "group-theory"},
			{"trait_type": "AI Cost (USD)", "value": "0.0200"},
			{"trait_type": "Strategy Used", "value": "visualize-group-theory"},
			{"trait_type": "Role", "value": "Visualizer"},
		},
	}
	status, resp = postJSON(t, fmt.Sprintf("/visualizations/%s/seal", vizID), nft)
	if status != http.StatusCreated {
		t.Fatalf("POST /visualizations/{id}/seal: status %d, body %v", status, resp)
	}
	if _, ok := resp["pr_url"]; !ok {
		t.Error("response missing pr_url")
	}

	// 3. GET /visualizations → verify vizID in list (rebuild may be async)
	waitFor(t, 5*time.Second, "visualization appears in GET /visualizations", func() bool {
		var vizs []struct {
			VisualizationID string `json:"visualization_id"`
		}
		getJSON(t, "/visualizations", &vizs)
		for _, v := range vizs {
			if v.VisualizationID == vizID {
				return true
			}
		}
		return false
	...
	for _, v := range vizs {
		if v.VisualizationID == vizID {
			if v.AuthorUsername != "viz-author" {
				t.Errorf("author_username = %q, want %q", v.AuthorUsername, "viz-author")
			}
			if v.ConjectureID != "prob_001" {
				t.Errorf("conjecture_id = %q, want %q", v.ConjectureID, "prob_001")
			}
			if v.Domain != "group-theory" {
				t.Errorf("domain = %q, want %q", v.Domain, "group-theory")
			}
			break
		}
	}

	// 4. GET /visualizations/{id} → verify full record including viz_json
	var viz struct {
		VisualizationID string  `json:"visualization_id"`
		AuthorUsername  string  `json:"author_username"`
		Title           string  `json:"title"`
	...
	getJSON(t, fmt.Sprintf("/visualizations/%s", vizID), &viz)
	if viz.VisualizationID != vizID {
		t.Errorf("visualization_id = %q, want %q", viz.VisualizationID, vizID)
	}
	if viz.Title != "Test Visualization" {
		t.Errorf("title = %q, want %q", viz.Title, "Test Visualization")
	}
	if viz.VizJSON == nil {
		t.Error("viz_json is nil")
	}
	if viz.CostUSD != 0.02 {
		t.Errorf("cost_usd = %f, want 0.02", viz.CostUSD)
	}
}
